...
class SpeedReduce : public BT::SyncActionNode
{   
    public:
        SpeedReduce(const std::string& name) : BT::SyncActionNode(name, {})
        {
            node = rclcpp::Node::make_shared("speed_reduce_bt_node");
            ...
            RCLCPP_INFO(node->get_logger(), "Speed reduce BT node started.");
        }
    ...
    {   
        ...
        // if (!rclcpp::ok())
        // {
        //     RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Interrupt while trying to call reduce speed service.");
        //     return BT::NodeStatus::FAILURE;
        // }
        // if (!service_client->wait_for_service(std::chrono::milliseconds(500)))
        // {
        //     RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Reduce speed service (CSU) is not available.");
        //     return BT::NodeStatus::FAILURE;
        // }

        auto result = service_client->async_send_request(request);

        RCLCPP_INFO(node->get_logger(), "Request for reduce speed service is sent.");
